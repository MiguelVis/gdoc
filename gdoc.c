...
/* Values in a comment block - step #1
   ---------------------------------------
*/
char *tag_file;      // @file
char *tag_fn;        // @fn
...
int flag_pre;                        // NZ if preformatted text is active
...
proc_file()
{
	...
	if(g_step && g_html) {
		...
		printf("pre {border-left:1px #000000 solid;padding:0.5em;background-color:#D0D0D0}");
		printf("</style>\n");
		printf("<head>\n");
		printf("<body>\n");
	}
	...
	while(read_line()) {
		...
		if(!strcmp(buf, g_doc_begin)) {
			...
			if(!strcmp(buf, g_doc_end)) {
				...
				doc = 0;   // We are now out a doc block
	...
	if(g_step) {
		...
		printf("%s %s %s - %s\n", TX_SIGNATURE, APP_NAME, APP_VERSION, APP_COPYRGT);
...
char *buf;
{
	...
	add_detail(flag_pre ? get_value(g_buf) : val);
}
...
dump_st1()
{
	...
	if(tag_file) {
		...
		header(TX_TT_UNKNOWN, 2, NULL);                   // Unknown -- FIXME
	}
	...
	if(tag_num_authors) {
		list(TX_AUTHOR, tag_authors, tag_num_authors);
	}

	// Version

	if(tag_version) {
		listOne(TX_VERSION, tag_version);
	}

	// Date

	if(tag_date) {
		listOne(TX_DATE, tag_date);
	}

	// Copyright

	if(tag_copyright) {
		listOne(TX_COPYRIGHT, tag_copyright);
	}
	...
	if(tag_num_details) {
		...
		// No list, no preformat, no paragraph
		...
		for(i = 0; i < tag_num_details; ++i) {
			...
			// Check for preformatted line

			if(!strcmp(pch, DET_PREFORMAT)) {
				if(pre) {

					// Pre already active, close it
...
*/
	...
	if(tag_file) {
		...
		if(ref_num_scs) {
			listDouble(TX_SECTIONS, ref_scs, NULL, ref_scslinks, ref_num_scs);
		}
	}
	...
	if(tag_num_doclinks) {
		listDouble(TX_LINKS, tag_docs, NULL, tag_doclinks, tag_num_doclinks);
	}
	...
	if(tag_file) {
		...
		if(ref_num_fns) {
			listDouble(TX_FUNCTIONS, ref_fns, ref_fnbriefs, ref_fnlinks, ref_num_fns);
		}
	...
	if(tag_num_params) {
		list(TX_PARAMETERS, tag_params, tag_num_params);
	}

	// Return

	if(tag_return) {
		listOne(TX_RETURNS, tag_return);
	}
}
